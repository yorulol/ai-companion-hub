...
function surfaceRows() {
  ...
  return [
    ...
    ["team share", s.enabled ? (s.token ? paint(C.green, `on :${s.port} (link printed below)`) : paint(C.yellow, "on but no token — run npm run setup")) : paint(C.grey, "off (TEAM_SHARE_ENABLED=false)"), C.reset],
